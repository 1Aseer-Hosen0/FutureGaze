...
import serviceAccountKey from "./futuregaze876.json" assert { type: "json" };
...
server.post("/signup", (req, res) => {
  ...
  // validating the data from fronted
  ...
  if (!passwordRegex.test(password)) {
    return res.status(403).json({
      error:
        "Password must be 8 to 20 characters long with a numeric, 1 lowercase and 1 uppercase letter",
    });
  }
  ...
  User.findOne({ "personal_info.email": email })
    .then((user) => {
      ...
      if (!user.google_auth) {
        ...
        return res.status(403).json({
          error:
            "Account was created using this google account. Try to login with google.",
        });
      }
    })
    .catch((err) => {
        ...
        (err, result) => {
          ...
          bcrypt.hash(newPassword, 10, (err, hashed_password) => {
            User.findOneAndUpdate(
              ...
              .catch((err) => {
                return res.status(500).json({
                  error: "Some error occurred while saving the new password.",
                });
              });
          });
        }
...
server.post("/update-profile", verifyJWT, (req, res) => {
  ...
  try {
    for (let i = 0; i < socialLinksArr.length; i++) {
      if (social_links[socialLinksArr[i]].length) {
        ...
        if (
          ...
          return res.status(403).json({
            error: `${socialLinksArr[i]} link is invalid. You must provide a valid social links.`,
          });
        }
      }
    }
    ...
    return res.status(500).json({
      error: "You must provide valid social links with http(s) include",
    });
  }
  ...
  User.findOneAndUpdate({ _id: req.user }, updateObj, {
    ...
    .catch((err) => {
      if (err.code == 11000) {
        return res
          .status(409)
          .json({ error: "Username is already taken. Try diffrent one." });
      }
      return res.status(500).json({ error: err.message });
    });
});
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (isAdmin) {
    ...
    if (!title.length) {
      return res
        .status(403)
        .json({ error: "You must need a title for your blog." });
    }
    ...
    if (!draft) {
      if (!des.length || des.length > 200) {
        return res
          .status(403)
          .json({ error: "You must provide blog description to publish it." });
      }

      if (!banner.length) {
        return res
          .status(403)
          .json({ error: "You must need a banner for your blog." });
      }
      ...
      if (!tags.length || tags.length > 10) {
        return res
          .status(403)
          .json({ error: "Provide tags for your blog. Maximum 10 Tags" });
      }
    }
